...
### [T-64] 07:50 WIB: Baca ulang seluruh dokumentasi untuk melanjutkan pengembangan
- Permintaan user: membaca semua dokumentasi proyek, terutama log sesi, secara teliti dan memahami posisi terakhir IRAMA sebelum melanjutkan pengembangan.
- Dibaca penuh: log ini (T-01 sampai T-63), README, `docs/kb/00`, `02`, `04`, `06`, `07`, `08`, `09`, `10`, `docs/planning/01` bagian 1 sampai 7, `02` sampai `16`, `06` bagian 1 sampai 8 beserta F02 dan F06, README kerangka kode, templat ADR, R00, R07, R08 (lokal), R05 bagian D, dan analisis majalah. KB-03 dibaca bagian A.11 sampai C; KB-05 kartu K-01 sampai K-08; KB-01, KB-11, dan R01 sampai R06 hanya daftar isinya karena isinya sudah diringkas di KB.
- Status repo: `main` sinkron dengan `origin/main` (commit terakhir `c39c7ac`), working tree bersih, kode belum ada.
- Temuan ketidakselarasan (belum diperbaiki, menunggu keputusan user):
  - `docs/adr/README.md` masih merujuk `06` §7 dan ADR-01 sampai ADR-18, serta mencatat keputusan lama (edge ringan di T2, TimescaleDB wajib, MQTT sejak awal). Yang berlaku: `06` §8, ADR-01 sampai ADR-25, tanpa edge sampai T4, TimescaleDB opsional, antrean PostgreSQL sampai T3.
  - Kolom tahap di KB-03 §B, KB-04, KB-05, KB-06 §F, KB-07, KB-08, dan R07 masih memakai skema tahapan lama (T1 MVP monitoring, T2 siap jual). Contoh: KB-REQ-005 override petugas dan KB-REQ-130 peta live tertulis T1, padahal menurut `04` dan `05` masing-masing T4 dan T3. SRS T1 sampai T2 harus memetakan ulang tahap menurut `04` dan `05`.
  - `docs/planning/10` §C masih menyebut `docs/kb/` sebagai salinan `_knowledge_base` yang disinkronkan manual; sejak T-25 `docs/kb/` adalah sumber kebenaran.
  - START_HERE langkah 1 merujuk status di bagian 9; catatan terbaru ada di bagian 10.
- Catatan teknis untuk planning T1: uji regresi contoh Dirjen 273/1996 (c = 70 s, C = 824, DS = 0,44) memerlukan kalkulator yang menerima arus dalam satuan SMP secara langsung dan set parameter Dirjen/MKJI (FP dengan wH normal 26 s, batas FUK), karena EMP sepeda motor PKJI 0,15 berbeda dari 0,2. Grafik tipe O, FG, dan NqMAX (F-T1-17, Q-15) tetap menjadi risiko T1.
- Catatan lingkungan: di Git Bash, `TZ=Asia/Jakarta date` menampilkan jam UTC; jam WIB diambil dari `date` biasa atau `Get-Date` PowerShell.
- Memori sesi di direktori memori baru dibuat ulang (preferensi penulisan dan cara bertanya, aturan tidak dipush, konteks proyek) karena direktori memori sesi ini kosong.

### [T-65] 09:58 WIB: Perbaikan temuan T-64, lalu commit dan push
- Permintaan user: memperbaiki dengan teliti dan hati-hati temuan T-64 yang belum sinkron, lalu commit dan push progres terakhir ke GitHub.
- **Temuan 1, daftar ADR.** `docs/adr/README.md` ditulis ulang: rujukan ke `06` bagian 8 (ADR-01 sampai ADR-25), ADR prioritas sebelum sprint 1, dan keputusan user yang belum ditulis sebagai ADR (ADR-01, 02, 04, 05, 06, 15, 19, 22, 23, 25) sesuai KB-10 bagian B dan B2. Keputusan lama (edge ringan T2, TimescaleDB wajib, MQTT sejak awal) dicatat sebagai sudah digantikan.
- **Temuan 2, kolom tahap KB dan R07.**
  - Skrip scratchpad `retag_kb.py` hanya mengganti sel kolom Tahap bila isi lamanya persis sama dengan yang diharapkan, dan tidak menulis apa pun bila ada yang tidak cocok. Hasil: KB-04 102 sel, KB-03 90 sel (57 di bagian A, 33 di matriks C-01 sampai C-43), KB-07 41 sel, KB-08 21 sel. Diff hanya menyentuh baris sasaran.
  - Acuan tahap per fitur adalah `05`; nilai yang berubah ditandai "(dulu …)" menurut skema 2026-09-12. Contoh: override petugas (KB-REQ-005, C-15) T4; aset dan pemeliharaan T3; notifikasi APILL tidak berfungsi T3 dari kamera dan T4 dari controller; nilai antara transparan (KB-REQ-111, 136, 166) T2; max-pressure, perimeter, Link Pivot, dan analitik probe T5; prioritas bus berbasis aturan T4 dan bersyarat T5.
  - Disunting manual: kepala KB-03, KB-04, KB-07, KB-08 (penjelasan skema tahap); KB-CON-02 (tanpa server sampai T3); KB-08 L-07, L-11, L-26, dan R-03; KB-05 bidang Tahap 12 kartu (K-07 dan K-11 tetap), cara uji K-01, catatan pada pohon keputusan dan hierarki cadangan (berlaku sejak kendali tersambung di T4); KB-06 bagian F ditulis ulang per tahap baru dengan API T1 sampai T2 dari `06` bagian 5; KB-09 pola lintas kota nomor 2; erratum di kepala R07. R01 sampai R06 tidak memakai tag tahap sehingga tidak diubah.
- **Temuan 3, dokumen 10.** Butir `docs/kb/` sebagai salinan `_knowledge_base` diganti (kini sumber kebenaran), enumerasi kode hi-res merujuk tabel resmi Purdue di KB-06, dan butir salinan regulasi ke `docs/kb/regulasi/` ditandai selesai karena regulasi sudah ada di `docs/sources/01_Regulasi_Indonesia/`.
- **Temuan 4, START_HERE.** Tanggal pembaruan, langkah baca 1 (bagian 9 dan 10), catatan tahap di KB, lembar fakta contoh resmi, dua workaround (jam Git Bash dan batas jalur scratchpad), tindak lanjut terbaru, dan erratum R07.
- **Temuan 5, uji kalkulator terhadap contoh resmi.** Saat memeriksa sumber ditemukan dua hal:
  - PKJI 2023 punya contoh resmi sendiri di Lampiran 12.5 (contoh 1 Jl. Iskandarsyah dan Jl. Wijaya, Jakarta, empat fase dan tiga fase dengan Tabel 12-9; contoh 2 perencanaan simpang baru). Nilai Formulir SA-IV dan SA-V di teks ekstraksi bertumpuk, jadi harus dibaca dari PDF.
  - Contoh Dirjen 273/1996 Bab X memakai pendekat terlawan dengan arus jenuh dasar dari grafik, serta EMP tahun 1996. Kendaraan tak bermotor dikonversi 0,5/1,0 dan ikut dihitung dalam arus; arus utara 363 SKR/jam termasuk 71 SKR/jam kendaraan tak bermotor. PKJI 2023 tidak mengonversi KTB, jadi kalkulator PKJI murni tidak akan menghasilkan C = 824 dan DS = 0,44 dari LHR.
  - Perubahan: `15` bagian 4.4 menjelaskan cara uji kedua contoh (PKJI dari data formulir; Dirjen dengan arus dan arus jenuh dari Formulir APILL-IV; uji Dirjen dari LHR bersama mode MKJI di T2) dan kriteria T1 di bagian 7. `04` definisi selesai T1 nomor 4, `05` F-T1-16 (masukan arus per kelas atau langsung dalam SMP), F-T1-160, dan F-T2-162, `13` D1.9, `01` istilah "Uji regresi", KB-05 K-01, KB-10 Q-11 dan Q-15, serta README `services/optimizer` dan `tests` ikut disesuaikan.
- **Temuan tambahan yang ikut diperbaiki.** `09` masih memakai penomoran ADR lama (ADR-001) dan kini memakai ADR-01 sampai ADR-05 dan ADR-19 sampai ADR-25. `05` F-T2-58 (T3) menyebut preemption details, sedangkan preemption baru ada di T4; deskripsinya kini menjelaskan bahwa bagian itu menyusul di T4. C-34 (T2) dan KB-REQ-020 (T4) sebelumnya berbeda untuk hal yang sama, kini keduanya T4.
- **Kendala.** Pencadangan ke scratchpad gagal pada berkas keempat (KB-08) karena jalur melebihi batas 260 karakter Windows. KB-03, KB-04, dan KB-07 sudah tertulis benar sebelum galat; KB-08 dijalankan ulang tanpa cadangan scratchpad karena versi asli ada di git. Uji ulang skrip pada ketiga berkas pertama menolak menulis, sesuai rancangan.
- **Pemeriksaan.**
  - Jumlah kolom tabel di 71 berkas markdown: tidak ada masalah.
  - Gaya sumber docx sederhana: tidak ada em-dash, en-dash, judul "dari X ke Y", atau judul pertanyaan. Dua kata "Bukan" di sumber sederhana 03 adalah negasi biasa yang sudah ada sejak 2026-09-14 dan tidak disentuh.
- **Versi lokal (tidak dipush).** Sumber sederhana 01, 04, 05, 13, dan 15 disesuaikan, lalu dibangun ulang dengan `--only` dan `--days 2` sehingga sandi dan tanggal berlaku tetap (17-09-2026). Verifikasi: kelima PDF terkunci AES-256 (V5, R6), sandi salah ditolak, sandi aktif membuka berkas, tanda air 17-09-2026 ada, dan teks baru ditemukan. Halaman 6 dokumen 15 dan halaman 3 dokumen 13 dirender lalu diperiksa; tampilannya rapi.
- **Commit dan push.** Semua perubahan kanonis di atas, termasuk entri T-64 dan T-65, di-commit dan dipush ke `origin/main` dalam satu commit. Berkas lokal (docx, PDF, sumber sederhana, naskah TA, R08) tetap di `.gitignore`.
