...
Status: siap pakai, 2026-09-13. Menjawab Q-01 (protokol & spesifikasi controller di kota pilot). Tujuan internal: menentukan adaptor mana yang dibangun di T2 (NTCIP vs serial vendor), apakah edge gateway diperlukan, dan apakah ada log/riwayat yang bisa dipakai untuk ATSPM.
...
- Hal yang **tidak ditanyakan karena sudah kita asumsikan** (Bagian I) — hanya dikonfirmasi bila jawaban lain mengindikasikan sebaliknya.
...
21. Tiga masalah teknis yang paling sering terjadi apa? (misalnya: jaringan putus, kamera rusak, controller hang, listrik padam, kabel dicuri/dirusak)
22. Ketika perangkat rusak, siapa yang memperbaiki (teknisi sendiri atau vendor)? Suku cadangnya mudah didapat?
23. Apa yang selama ini diinginkan dari sistem tetapi belum bisa dilakukan?

## F. Dokumen dan kontak

24. Apakah ada buku manual/dokumen teknis controller dan sistem ruang kendali yang bisa kami pelajari (bisa dipinjam/difoto)?
25. Boleh kami diperkenalkan ke kontak teknis vendor perangkat yang sekarang dipakai?

## G. Kerja sama uji coba (opsional, bila suasana mendukung)

26. Kalau ada perangkat lunak pemantauan tambahan yang tidak mengganggu sistem yang ada (hanya membaca), apakah dimungkinkan diuji di 3–5 simpang dalam satu koridor? Apa syarat administrasinya (surat, nota kesepahaman, izin akses)?
27. Dengan siapa hal ini sebaiknya dibicarakan lebih lanjut?
...
| Komunikasi ke ruang kendali via fiber/Metro-E untuk pusat kota dan modem 4G untuk pinggiran | Pola ATCS kota-kota Indonesia (KB-09) | Bila hanya 4G → buffer edge & agregasi wajib sejak T2 |
| CCTV IP standar (RTSP/ONVIF), disimpan di NVR vendor, tanpa analitik hitung kendaraan pada sebagian besar simpang | Surabaya 580 kamera/8 sensor adaptif; Bandung CCTV publik | Bila kamera analitik ada → konsumsi keluaran vendor (U-08 hybrid) |
...
| Tidak ada log hi-res 0,1 s dari controller; riwayat hanya di server vendor (bila ada) | Umum di ATCS Indonesia | Bila ada → ATSPM langsung dari log; tanpa itu → edge logger (F-T2-31) |
