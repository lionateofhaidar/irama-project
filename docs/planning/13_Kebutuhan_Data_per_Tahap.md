...
| D1.7 | **Geometri tiap pendekat** | CSV `pendekat.csv`: lebar pendekat, lebar masuk, lebar keluar, lebar lajur belok kiri langsung, jumlah lajur, median, kelandaian, jarak parkir terdekat dari garis henti, foto tiap lengan | ukur dari citra satelit, lalu verifikasi dengan satu kunjungan singkat memakai meteran atau aplikasi ukur | semua pendekat simpang target | kalkulator PKJI (arus jenuh, lebar efektif) |
