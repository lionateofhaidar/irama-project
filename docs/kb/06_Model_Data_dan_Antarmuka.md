...
- **T1 (MVP monitoring + plan mgmt + KPI):** Inventory `GET/PUT /intersections, /controllers, /phases, /rings, /overlaps, /channels, /detectors, /patterns, /splits, /timebase/actions, /dayplans, /schedules`; Live `GET /controllers/{id}/status`, `WS …/status/stream`, `/detectors/status`; Assets/Maintenance `/assets, /maintenance/schedules, /workorders`; Alerts `GET /alerts`, `POST /alerts/{id}/ack`; KPI dasar `GET /kpi/{intersection}/pkji`, `/health`; Audit `GET /audit`.
- **T2 (siap jual):** Command `POST /controllers/{id}/commands/pattern|sync|special-function|unit-control` (dengan heartbeat otomatis); Plan `POST /controllers/{id}/transactions`, `PUT /transactions/{tid}/objects|blocks`, `POST …/verify|commit|abort`, `GET/POST /controllers/{id}/blocks/{dataID}`; ATSPM `POST /hires/ingest`, `GET /atspm/{i}/phase-termination|split-monitor|ped-delay|preemption` (tanpa detektor tambahan); Corridor `GET /corridors/{id}/summary`, `/compare?before&after`; Public `GET /public/kpi`, `/public/intersections`; Service requests `/service-requests`.
- **T3 (transisi adaptif):** `POST /controllers/{id}/commands/phase-control|ring-control|preempt`; `POST /priority/requests`; `POST /corridors/{id}/green-wave`; `POST /corridors/{id}/link-pivot/run`; Sim `POST /sim/scenarios`, `POST /sim/runs`, `GET /sim/runs/{id}/atspm/...`; ATSPM lengkap `/pcd|split-failure|approach-delay|arrivals-on-red|tmc|approach-volume|yra`; External ICD endpoints (ETLE evidence push, AVL ingest, CAD ingest).
- **T4 (setara ITCS):** adaptive engine endpoints (`/adaptive/groups`, `/adaptive/params`, `/adaptive/decisions` transparan), perimeter control, oversaturation plans, objective per grup, external command intake (ICM).
- **T5 (end-state):** RL advisor (`/advisor/recommendations`, shadow logs), digital twin 3D/prediksi, SPaT/MAP, ERP/KRE data feeds.
